...
Therefore, Horn clauses are particularly suited for implementing [formal systems](https://en.wikipedia.org/wiki/Formal_system): it turns textbook definitions of logical systems, models of computation, and programming languages into a fully oprerational implementation. To try `horc` on any computer preinstalled with [Docker](https://en.wikipedia.org/wiki/Docker_(software)), run for example: `docker run --rm amka66/horc list.hn list-queries.pl`.
